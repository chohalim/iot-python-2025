...
print(greeting +' '+ target) # 문자열 연산 + string concatenate(연쇄)
...
year = current[0:3 + 1]
month = current[5:6 + 1]
day = current[8:] # end 끝까지 수는 생략 가능
print(year, month, day)
print(current[-2:])

## 문자열 연산 중 함수를 사용 # fullNameString 카멜(낙타)케이싱 full_name 스네이크(뱀)케이싱
full_name ="Hugo MG. Sung"
print(full_name.split()) # 기본 공백으로 잘라라
print(full_name.split(' ')) # 공백기준으로 잘라라

names = full_name.split(' ')
print(type(names)) # split 한 값의 type은 list
print(names)

names = full_name.split('.') # . 기준으로 잘라라
print(type(names))
print(names)
...
print(f'//{origin.strip()}//') # 문자열 중간 공백은 제거X

# 단어찾기
print(full_name.find('G'))
print(full_name.find('g'))
print(full_name.find('h')) # -1나옴 : h를 찾을 수 없음

print(full_name.count('g')) # g가 문장에 몇번 존재
print(full_name.index('g')) # 몇번째에 g가 존재하는지
# print(full_name.index('h')) # 오류발생

print(full_name.upper()) # 전부 대문자로
print(full_name.lower()) # 전부 소문자로
