...
# 버튼 위젯
button = Button(root, teat='클릭', command=buttonClick)
button.pack()
...
choice = IntVar() # 초기화면에 아무것도 선택되어 있지 않게 하기 위해
...
orange = IntVar()
Checkbutton(root, text='오렌지', variable=choice).pack(anchor=W)
banana = IntVar()
Checkbutton(root, text='바나나', variable=choice).pack(anchor=W)
mango = IntVar()
Checkbutton(root, text='망고', variable=choice).pack(anchor=W)
...
button4 = Button(frame, text = '버튼', width=20)
